...
namespace MyWeatherAppp.ViewModels
{
        internal class WeatherViewModel: Base.BaseViewModel
        {
            ...
            //RefreshDataCommand = new RelayCommand(async (o) => await RefreshData());
                RefreshDataCommand = new RelayCommand(
                    async (o) => await RefreshData(),
                    (o) => SettingsService.Configuration.SelectedLocation != null
                );
            ...
            private async Task RefreshData()
            {
                ...
                {
                    Forecast = await WeatherService.Get3DayForecastAsync(
                        SettingsService.Configuration.SelectedLocation.Latitude, 
                        SettingsService.Configuration.SelectedLocation.Longitude, 
                        SettingsService.Configuration.SelectedLocation.Name
                        )!;
                    if (Forecast == null)
                    {
            ...
            public override void OnAppearing()
            {
                ...
                if (Forecast.Measures.PrecipitationUnit != SettingsService.Configuration.Lenght ||
                    Forecast.Measures.TemperatureUnit != SettingsService.Configuration.Temperature ||
                    Forecast.Measures.WindSpeedUnit != SettingsService.Configuration.Wind ||
                    Forecast.Measures.PressureUnit != SettingsService.Configuration.Pressure)
                    RefreshDataCommand.Execute(null);
        ...
        private void BuildTempGraph()
        {
            ...
            {
                Name = App.Current.Resources[Forecast.Measures.TemperatureUnit.ToString()].ToString(),
            ...
            {
                series.Values = Forecast.DayForecasts.Select(p => Math.Round(p.MaxTemperature, 3, MidpointRounding.AwayFromZero)).ToList();

                Series = new ISeries[2] { series,  new LineSeries<double>
                {
                    Name = App.Current.Resources[Forecast.Measures.TemperatureUnit.ToString()].ToString(),
                    Values = Forecast.DayForecasts.Select(p => Math.Round(p.MinTemperature, 3, MidpointRounding.AwayFromZero)).ToList(),
                    Stroke =  new LinearGradientPaint(
        ...
        private void BuildPressureGraph()
        {
            ...
            var series = new LineSeries<double>
            {
                Name = App.Current.Resources[Forecast.Measures.PressureUnit.ToString()].ToString(),
                Stroke = new LinearGradientPaint(
            ...
            if (IsWeekTimeSelected)
            {
                series.Values = (IReadOnlyCollection<double>?)Forecast.DayForecasts.Select(p => Math.Round(p.HourlyForecasts.Average(h => h.SurfacePressure),3,MidpointRounding.AwayFromZero)).ToList();
                return;
            }
            series.Values = Forecast.DayForecasts.Select(p => Math.Round(p.MaxTemperature, 3, MidpointRounding.AwayFromZero)).ToList(); 
        }
        ...
        private void BuildHumidityGraph()
        {
            ...
            if (IsWeekTimeSelected)
            {
                series.Values = (IReadOnlyCollection<double>?)Forecast.DayForecasts.Select(p => Math.Round(p.HourlyForecasts.Average(h => h.RelativeHumidity), 3, MidpointRounding.AwayFromZero)).ToList();                
                return;
            }
            series.Values = Forecast.DayForecasts.Select(p => Math.Round(p.MaxTemperature, 3, MidpointRounding.AwayFromZero)).ToList();
        }
